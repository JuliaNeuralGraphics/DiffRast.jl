...
function CameraKeyframe(R, t)
    q = QuatRotation{Float32}(R)
    CameraKeyframe(QuaternionF32(q.w, q.x, q.y, q.z, true), t)
end
...
"""
    get_rotation(k::CameraKeyframe)

Get `3x3` rotation matrix from `CameraKeyframe`, which stores rotation
as unit quaternion.
"""
function get_rotation(k::CameraKeyframe)
    q = QuatRotation{Float32}(k.q.s, k.q.v1, k.q.v2, k.q.v3, k.q.norm)
